...
class SignUp extends Component{
  ...
  handleSubmit=(e)=>{
     ...
     let phonenumber="+91"+this.state.tel;
     firebase.auth().signInWithPhoneNumber(phonenumber,recaptcha).then((resp)=>{

                     let code=prompt("Dnter the 6-digit otp code","");
                     if(code==null)return;
                     resp.confirm(code)
                   }).then(()=>{
                     console.log("success");
                   }).catch((err)=>{
                     alert("Fail to Connect.Refresh the page and try again");
                   })
          }else{
            alert("Please Enter a Correct Phone number with out using any country code like +91 and 0")
          }
  }
  render(){
